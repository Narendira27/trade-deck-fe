...
const StatusIndicator: React.FC<StatusIndicatorProps> = ({ onClick }) => {
  const [overallStatus, setOverallStatus] = useState<'good' | 'warning' | 'error'>('error');
  ...
  const checkQuickStatus = async () => {
    ...
    try {
      // Quick health checks
      const backendPromise = axios.get(`${API_URL}/health`, {
        headers: { Authorization: `Bearer ${auth}` },
        timeout: 3000,
      }).then(() => true).catch(() => false);

      const mainSocketPromise = fetch(`${SOCKET_MAIN}/health`, { 
        signal: AbortSignal.timeout(3000) 
      }).then(res => res.json())
        .then(data => data.brokerWSConnected && data.redisConnected)
        .catch(() => false);

      const feSocketPromise = fetch(`${SOCKET_FE}/health`, { 
        signal: AbortSignal.timeout(3000) 
      }).then(res => res.json())
        .then(data => data.brokerWSConnected && data.redisConnected)
        .catch(() => false);

      const [backendStatus, mainSocketStatus, feSocketStatus] = await Promise.all([
        backendPromise,
        mainSocketPromise,
        feSocketPromise,
      ]);

      const workingServices = [backendStatus, mainSocketStatus, feSocketStatus].filter(Boolean).length;
